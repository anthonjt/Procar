...
int main(void)
{
    ...
    BITBAND_REG(GPIOC->PDDR, 2) = 1;     //Make PTD pin 1 output
    ...
    // before the pulse width command may be altered from FTM3->CONTROLS[1].CnV
    ...
    FTM0->SC  |= FTM_SC_PS(FTM0_CLK_PRESCALE); //divide the input clock down by 2^FTM3_CLK_PRESCALE
    FTM0->SC  |= FTM_SC_CLKS(FTM0_CH1_CLK_SOURCE);
