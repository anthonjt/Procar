...
void PWM_initfun(void){
    ...
    BITBAND_REG(GPIOC->PDDR, 10) = 1;     //Make PTC pin 10 output
    FTM3->SC = 0;  						// Make sure its Off!
    FTM3->CNTIN = 0;  				    // starting value for counter
    FTM3->CNT= 0; 						// loads the counter with CNTIN
    FTM3->CONTROLS[6].CnSC = FTM_CnSC_MSB_MASK | FTM_CnSC_ELSB_MASK; // FTM3_CH6
    ...
    FTM3->SC  |= FTM_SC_PS(FTM3_CLK_PRESCALE); 		// Divide the input clock down by 2^FTM3_CLK_PRESCALE
